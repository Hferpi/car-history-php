...
@section('content')
    <div class="w-full p-6">
        ...
        <section class="w-full items-center">
            @if($ultimoVehiculo)
                <div class="w-full rounded-2xl border border-gray-200 dark:border-gray-700 flex flex-col items-center bg-white dark:bg-gray-800 shadow-sm overflow-hidden">
                    ...
                    <div class="bg-sky-200 dark:bg-gray-600 w-full p-4 items-center flex justify-around">
                        <h3 class="text-2xl dark:text-white">
                            <span class="font-bold text-blue-600 dark:text-blue-400 text-sm block uppercase">{{ $ultimoVehiculo->marca }}</span>
                            <span>{{ $ultimoVehiculo->modelo->nombre }}</span>
                        </h3>

                        <!-- MATRÍCULA ESTILO REAL -->
                        <div class="flex items-stretch bg-white rounded border border-gray-400 overflow-hidden h-10 shadow-sm">
            ...
            @else
                <div class="w-full p-10 text-center border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-2xl">
        ...
        <div class="relative w-full mt-10 p-5 bg-sky-200 dark:bg-gray-600 rounded-2xl grid grid-cols-3 place-items-center text-center">
            <h4 class="absolute -top-4 left-1/2 -translate-x-1/2 bg-sky-400 dark:bg-sky-500 px-4 py-1.5 rounded-full text-sm font-semibold shadow">
                Último servicio
            </h4>
            ...
            <div class="mt-2">
                <h3 class="font-bold">{{ $repair['tipo_servicio'] }}</h3>
                <p class="text-sm">{{ $repair['fecha'] }} • {{ $repair['km'] }}km</p>
            </div>

            <p class="font-bold text-lg">{{ $repair['precio'] }}€</p>
        </div>
        ...
        <div class="text-center w-full bg-sky-200 dark:bg-gray-600 rounded-2xl mt-8 p-6">
            ...
            <p class="text-3xl text-red-600 font-bold">3.450€</p>
        </div>
    </div>
@endsection